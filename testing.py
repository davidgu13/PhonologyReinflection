...
from hyper_params_config import training_mode, inp_phon_type, out_phon_type, PHON_REEVALUATE, PHON_UPGRADED, ANALOGY_MODE,\
    analogy_type, data_dir,  learning_rate, encoder_embedding_size, train_file, dev_file, test_file, \
...
def main():
    ...
    print(f"\nConfiguration: split-type = {training_mode}, inp_phon_type = {inp_phon_type}, analogy_mode = {ANALOGY_MODE}\n"
          f"Trained on file: {os.path.join(data_dir, train_file)}\n"
          f"Validation file: {os.path.join(data_dir, dev_file)}\n"
    ...
    if out_phon_type!='graphemes' and not PHON_UPGRADED:
        result_phon, accuracy_phon, result_morph, accuracy_morph = bleu(dev_data, model2, srcField, trgField, device, converter=combined, output_file=preds_file)
