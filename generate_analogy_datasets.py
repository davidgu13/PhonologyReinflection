...
# from hyper_params_config import training_mode # , lang, POS, analogy_type
import codecs

lang, POS = 'kat', 'V'
analogy_type = 'src_cross1'
random.seed(1)
...
class AnalogyFunctionality(Data2SamplesConverter):
    ...
    def _generate_cross_1_datasets(self):
        """
        ...
        for orignal_samples_list in orignal_samples_lists:
            ...
            for reinflection_sample in tqdm(orignal_samples_list):
                ...
                new_sample = ((src_feat, src_form, trg_feat, aux_src_form, aux_trg_form), trg_form)
    ...
    def generate_data(self):
        return self._generate_cross_1_datasets()
...
def main():
    ...
    for training_mode in ['form', 'lemma']:
        # original_files = [f"{training_mode}sp.schema2.{e}.txt" for e in ['train', 'dev', 'test']]
        ...
        analogised_train_dev_test_data = kat_analogies.generate_data() # analogised_train_dev_test_data is a [train, dev, test] list of datasets
...
def auxiliary_main():
    ...
    # original_dir = join(".data", "OriginalData")
    analogies_dir = join(".data", "AnalogiesData", analogy_type)
    inflections_file = join(".data", "OriginalData", "Inflection Tables Georgian.txt")
    original_train_file, original_dev_file, original_test_file = [f"{training_mode}sp.schema2.{e}.txt" for e in ['train', 'dev', 'test']]

    kat_analogies = AnalogyFunctionality(analogies_dir, inflections_file, original_train_file, original_dev_file, original_test_file)

    train_analog_path = join(analogies_dir, f"{training_mode}sp.schema2.train_{analogy_type}.tsv")
...
# inflections_file = join(".data", "OriginalData", "Inflection Tables Georgian.txt")
# original_train_file, original_dev_file, original_test_file = [f"{training_mode}sp.schema2.{e}.txt" for e in ['train', 'dev', 'test']]
# analogies_converter = AnalogyFunctionality(original_dir, analogies_dir, inflections_file, original_train_file, original_dev_file, original_test_file)
